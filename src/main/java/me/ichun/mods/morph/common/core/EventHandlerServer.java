...
public class EventHandlerServer {
    ...
    @SubscribeEvent public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            if (info.getCurrentState() != null && info.getCurrentState().variant != null) {
                ...
                if (!player.level().isClientSide && player.level().isDay() && !player.level().isRaining()) {
                    ...
                    if (isUndead) {
                        ...
                        if (f > 0.5F && player.level().canSeeSky(pos)) {
                            if (player.getItemBySlot(EquipmentSlot.HEAD).isEmpty()) { player.setSecondsOnFire(8); }
                        }
                    }
                }
            }
    ...
    @SubscribeEvent public void onLivingAttack(LivingAttackEvent event) {
        if (event.getEntity() instanceof Player player) {
            ...
            if (info.getCurrentState() != null && info.getCurrentState().variant != null) {
                ...
                if (isFireProof && (event.getSource().is(net.minecraft.world.damagesource.DamageTypes.IN_FIRE) || event.getSource().is(net.minecraft.world.damagesource.DamageTypes.ON_FIRE) || event.getSource().is(net.minecraft.world.damagesource.DamageTypes.LAVA) || event.getSource().is(net.minecraft.world.damagesource.DamageTypes.HOT_FLOOR))) {
    ...
    @SubscribeEvent public void onTargetChange(LivingChangeTargetEvent event) {
        if (event.getNewTarget() instanceof Player player && event.getEntity() instanceof Mob mob) {
            MorphInfo info = MorphApi.getApi().getMorphInfo(player);
            if (info.getCurrentState() != null && info.getCurrentState().variant != null) {
                if (info.getCurrentState().variant.id.equals(ForgeRegistries.ENTITY_TYPES.getKey(mob.getType()))) {
                    event.setCanceled(true);
                }
            }
        }
